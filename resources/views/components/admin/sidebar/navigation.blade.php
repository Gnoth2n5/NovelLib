...
$menuItems = [
    ...
    // Hàm kiểm tra xem một submenu có đang active không
    function isSubmenuActive($submenu)
    {
        return request()->routeIs($submenu['route']);
    }

    // Hàm kiểm tra xem một menu có submenu đang active không
    function hasActiveSubmenu($menu)
    {
...
<ul class="space-y-2">
    @foreach ($menuItems as $item)
        @if ($item['type'] === 'dropdown')
            <li>
                <details class="group" {{ hasActiveSubmenu($item) ? 'open' : '' }}>
                    <summary class="flex cursor-pointer items-center justify-between px-4 py-3 rounded-lg hover:bg-base-200 {{ hasActiveSubmenu($item) ? 'bg-primary text-white font-medium' : '' }}">
                        ...
                        <svg class="h-4 w-4 shrink-0 transition duration-300 group-open:-rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                        </svg>
                    </summary>
                    <ul class="mt-2 space-y-2 px-8">
                        @foreach ($item['submenu'] as $subItem)
                            <li>
                                <a href="{{ route($subItem['route']) }}" 
                                   class="flex items-center gap-3 py-3 px-4 rounded-lg {{ request()->routeIs($subItem['route']) ? 'bg-primary text-white font-medium' : 'hover:bg-base-200' }}"
                                >
        ...
        @else
            <li>
                <a href="{{ route($item['route']) }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs($item['route']) ? 'bg-primary text-white font-medium' : 'hover:bg-base-200' }}"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        {!! $item['icon'] !!}
                    </svg>
